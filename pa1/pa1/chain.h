...
class Chain {
 ...
 private:
  ...
  void swapvectorval(int i, int j, std::vector<double> & A);
  void setvector(std::vector<double> & B_values, ::vector<int> & indices);
  Node * getNodeAt(int i);
  int findMax(std::vector<double> & b_vector, int start_index);
